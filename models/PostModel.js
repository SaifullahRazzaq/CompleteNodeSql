...
class Post {
    ...
    static getPostByDate(start, end, result)
    {
       
        sql.query("Select * from posts where create_date between ? AND ? ORDER BY create_date", [start,end], function (err, res) {
            if (err) {
                console.log("error: ", err);
                result(err, null);
            }
    ...
    static getPostByCategory(category_id, result)
    {
        var sqlQuery = "select * from  posts where category_id = ?";
        sql.query(sqlQuery, [category_id], function (err, res) {
            if (err) {
                console.log("error: ", err);
                result(err, null);
            }
